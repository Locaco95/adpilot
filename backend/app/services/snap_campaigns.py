"""Snap campaign write operations, shared by the web API and the Telegram agent.

create_full_campaign runs the whole chain — media → upload → creative →
campaign → ad squad → ad — everything PAUSED so nothing spends until
explicitly activated. The update helpers follow Snap's read-modify-PUT
convention (collection path, array body, object carries its id).
"""
...
async def update_adsquad_budget(ad_squad_id: str, daily_budget: float) -> dict:
    """Change an ad squad's daily budget (read-modify-PUT)."""
    client = get_snap_client()
    data = await client.get(f"/adsquads/{ad_squad_id}")
    obj = _first(data, "adsquads")
    obj["daily_budget_micro"] = int(round(daily_budget * 1_000_000))
    resp = await client.put(
        f"/campaigns/{obj['campaign_id']}/adsquads",
        json={"adsquads": [obj]},
    )
    return _first(resp, "adsquads")


async def set_campaign_status(campaign_id: str, status: str) -> dict:
    """Set a campaign ACTIVE or PAUSED (read-modify-PUT)."""
    if status not in ("ACTIVE", "PAUSED"):
        raise HTTPException(422, f"Invalid status {status}; use ACTIVE or PAUSED.")
    s = get_settings()
    client = get_snap_client()
    data = await client.get(f"/campaigns/{campaign_id}")
    obj = _first(data, "campaigns")
    obj["status"] = status
    resp = await client.put(
        f"/adaccounts/{s.snapchat_ad_account_id}/campaigns",
        json={"campaigns": [obj]},
    )
    return _first(resp, "campaigns")
