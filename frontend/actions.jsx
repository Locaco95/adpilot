...
function ActionsPage({ isLoading = false }) {
  ...
  const filteredActions = PENDING_ACTIONS.filter(a => {
    if (filter === 'all') return true;
    if (filter === 'pending') return !a.status && !actionStates[a.id];
    if (filter === 'tier3') return a.tier === 3;
    if (filter === 'tier2') return a.tier === 2;
    return true;
  });
  ...
  return (
    <div>
      ...
      <div className="flex gap-12 mb-16 fade-in" style={{ gap: 12, marginBottom: 16 }}>
        <ActionSummaryCard
          label="Tier 3 — Awaiting" count={PENDING_ACTIONS.filter(a => a.tier === 3 && !a.status).length}
          color="var(--danger)" icon="⬤" />
        <ActionSummaryCard
          label="Tier 2 — Auto-executed" count={PENDING_ACTIONS.filter(a => a.tier === 2).length}
          color="var(--warning)" icon="◐" />
        <ActionSummaryCard
          label="Tier 1 — Autonomous" count={4}
      ...
      <div className="tab-bar fade-in fade-in-1">
        {[
          ...
          <button key={t.id} className={`tab-btn ${filter === t.id ? 'active' : ''}`}
      ...
      <div className="card mt-16 fade-in fade-in-3" style={{ marginTop: 20 }}>
...
/* ── Action Summary Card ──────────────────────────────── */
function ActionSummaryCard({ label, count, color, icon }) {
  return (
...
/* ── Action Card ──────────────────────────────────────── */
function ActionCard({ action, state, onAction }) {
  const [expanded, setExpanded] = useState(false);
  const isResolved = state === 'approved' || state === 'rejected' || action.status === 'auto_approved';
  const impactColors = { high: 'var(--accent)', medium: 'var(--info)', low: 'var(--text-tertiary)' };

  return (
    <div className="card" style={{
      borderLeft: `3px solid ${action.tier === 3 ? 'var(--danger)' : action.tier === 2 ? 'var(--warning)' : 'var(--success)'}`,
      ...
      <div className="flex items-center justify-between" style={{ marginBottom: 8 }}>
        <div className="flex items-center gap-8" style={{ gap: 10 }}>
          ...
          <span style={{
            ...
            color: impactColors[action.impact], textTransform: 'uppercase',
          }}>
        ...
        <div className="flex items-center gap-8" style={{ gap: 8 }}>
          ...
          {state === 'approved' && <span className="badge badge-success">✓ Approved</span>}
          {state === 'rejected' && <span className="badge badge-danger">✗ Rejected</span>}
          <span className="text-xs text-tertiary">{timeAgo(action.createdAt)}</span>
        </div>
      </div>
      ...
      <button onClick={() => setExpanded(!expanded)}
        style={{
          ...
          fontSize: 12, fontFamily: 'var(--font-body)', fontWeight: 500, padding: '4px 0',
        }}>
      ...
      {expanded && (
        <div style={{
          ...
          padding: '10px 12px', marginTop: 6, marginBottom: 8,
      ...
      {!isResolved && action.tier === 3 && (
        <div className="flex gap-8 mt-8" style={{ gap: 8, marginTop: 10 }}>
          <button className="btn btn-success btn-sm" onClick={() => onAction(action.id, 'approved')}>
            ✓ Approve
          </button>
          <button className="btn btn-danger btn-sm" onClick={() => onAction(action.id, 'rejected')}>
            ✗ Reject
          </button>
      ...
      {action.status === 'auto_approved' && !state && (
...
function RuleCard({ title, color, rules }) {
  return (
    <div style={{
      background: 'var(--bg-surface)', borderRadius: 'var(--radius-sm)',
      padding: '12px 14px', border: `1px solid ${color}22`,
    }}>
...
Object.assign(window, { ActionsPage, ActionSummaryCard, ActionCard, RuleCard });
